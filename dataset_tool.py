...
def open_image_folder(source_dir, *, max_images: Optional[int]):
    ...
    # added by authors
        ...
        images = ['/'.join(input_img.split('/')[-2:]) for input_img in input_images]
        dirs = [img.split('/')[0] for img in images]
        label_name = set(dirs)
        dir2label = dict(zip(label_name, range(len(label_name))))
        input_labels = [dir2label[dir] for dir in dirs]
        labels = dict(zip(images, input_labels))
...
# added by authors
def open_cifar100(tarball: str, *, max_images: Optional[int]):    # Added by the authors
    images = []
    labels = []
...
def open_dataset(source, *, max_images: Optional[int]):
    if os.path.isdir(source):
        ...
        # added by authors
        if os.path.basename(source) == 'cifar-100-python.tar.gz':
