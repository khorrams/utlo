...
class Dataset(torch.utils.data.Dataset):
    ...
    # added by authors
    @property
    def shot_labels(self):
        out = []

        if self.label_dim == 10:  # CIFAR-10
            inds = [0, 3, 6, 10]
        elif self.label_dim == 100:  # CIFAR-100
            inds = [0, 35, 70, 100]
        elif self.label_dim == 102:  # Flowers
            inds = [0, 20, 50, 102]
        elif self.label_dim == 5:  # LSUN-5
            inds = [0, 3, 4, 5]
        elif self.label_dim == 20:  # Animals
            inds = [0, 5, 10, 20]
        else:
            raise ValueError("dataset not valid.")

        for i in range(len(inds) - 1):
            out.append(list(range(inds[i], inds[i + 1])))
        return out
#----------------------------------------------------------------------------
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        if os.path.isdir(self._path):
            ...
            if super_kwargs["fname"]:
                ...
                raise ValueError("--fname must be specified as an input arg.")
            self._type = 'zip'
